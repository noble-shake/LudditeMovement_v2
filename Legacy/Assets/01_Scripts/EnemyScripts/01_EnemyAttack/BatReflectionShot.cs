...
public class BatReflectionShot : IEnemyAttack
{
    ...
    public IEnumerator Attack()
    {
        ...
        int halfIndex = 3 / 2; // 1, 2, 3, 4
        angle -= halfIndex * 15f;

        tempAngle = angle;
        for (int i = -halfIndex; i < 3 - halfIndex; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                ...
                bullet.bulletSprite.transform.localScale = new Vector3(1f - j * 0.3f, 1f, 1f);
