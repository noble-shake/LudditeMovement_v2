...
public class GridPathfinding
{
    ...
    public GridPathfinding(Vector3 worldLowerLeft, Vector3 worldUpperRight, float nodeSize = 1f)
    {
        Instance = this;
        worldOrigin = worldLowerLeft;
        this.nodeSize = nodeSize;

        // new Vector3(-8.5f, 0f, -4f), new Vector3(8f, 0f, 4f)
        float worldWidth = worldUpperRight.x - worldLowerLeft.x;
        float worldHeight = worldUpperRight.z - worldLowerLeft.z;
    ...
    public void SetWalkable((int, int) Edges, TileCompression _tileComp)
    {
        if (_tileComp.tile == null)
        {
            mapNodes[Edges.Item1][Edges.Item2].SetWalkable(false);
        }
        else
        {
            if (_tileComp.tile.GetTileType() == TileType.Sand)
            {
                if (_tileComp.building == null)
                {
                    mapNodes[Edges.Item1][Edges.Item2].isSand = true;
                    mapNodes[Edges.Item1][Edges.Item2].SetWalkable(true);
                }
                else
                {
                    mapNodes[Edges.Item1][Edges.Item2].isSand = true;
            ...
            else if (_tileComp.tile.GetTileType() == TileType.Road)
            {
                if (_tileComp.building == null)
                {
                    mapNodes[Edges.Item1][Edges.Item2].SetWalkable(true);
    ...
    public void SetWalkable(TileBuildType[] Tiles, PropType[] Buildings)
    {
        int row = 9;
        int col = 16;
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                {
                    mapNodes[j][i].SetWalkable(false);
                }
                else
                {
                    if (Tiles[index] == TileBuildType.Sand)
                    {
                        if (Buildings[index] == PropType.None)
                        {
                            mapNodes[j][i].isSand = true;
                            mapNodes[j][i].SetWalkable(true);
                        }
                        else
                        {
                            mapNodes[j][i].isSand = false;
                            mapNodes[j][i].SetWalkable(false);
                        }

                    }
                    else if (Tiles[index] == TileBuildType.Road)
                    {
                        if (Buildings[index] == PropType.None)
                        {
                            mapNodes[j][i].SetWalkable(true);
    ...
    public List<PathNode> GetPath(Vector3 start, Vector3 end)
    {
        ...
        MapPos startMapPos = GetClosestValidPos(Mathf.RoundToInt(start.x), Mathf.RoundToInt(start.z));
        MapPos endMapPos = GetClosestValidPos(Mathf.RoundToInt(end.x), Mathf.RoundToInt(end.z));
    ...
    public List<PathNode> findPath(int startX, int startY, int endX, int endY)
    {
        ...
        // List<PathNode> HeuristicList = new List<PathNode>();
        ...
        PathNode MemoryNode = mapNodes[startX][startY];
    ...
    private void CalculateManhattanDistance(PathNode currentNode, int currX, int currY, int targetX, int targetY)
    {
        ...
        currentNode.hValue = (int)((Mathf.Abs(currX - targetX) + Mathf.Abs(currY - targetY)) * (currentNode.isSand ? 2f : 1f));
        currentNode.isOnOpenList = false;
